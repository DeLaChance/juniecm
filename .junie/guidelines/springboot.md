...
* Use `@RequiredArgs` to not have a constructor
...
## 5. SQL and database logic
* Do not use Spring Data JPA
* Use JDBC instead with `JdbcTemplate`
* Load SQL queries from `/src/main/resources/sql` directory, via the classpath at constructor startup.
...
## 8. Use Request Objects for Business Operations
* Create purpose-built request records (e.g., `AddConfigurationItemRequest`) to wrap input data.
* Accept these requests in your service methods to drive creation or update workflows.
...
* Do not return stack straces in the HTTP response.
...
## 15. Mapping Entity to DTO

- Each database entity will map a table in the database
- It will have a static `from()` method that is consuming a `Map<String, Object>` and creates the object
- The entity will map to the DTO, only showing the fields relevant to the frontend. Derived properties are generated in the DTO, e.g a hyperlink.
- Mapping between entity and DTO happens in a `from()` method on the DTO.
- Do not use MapStruct or any other framework. Write plain Java code.

## 11. Versioning
- Use Spring Boot starter version 4.0.2.